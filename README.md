...
| `replaces` | string | optional | gpk-only | What the mod overrides in game (costume/item names + IDs, UI window, mount, face decal row) ending with a "Check:" hint telling the user what to look at. Derived from the GPK name table, the datacenter item cross-ref, or the original post — package names quoted are ground truth. |
...
| `last_verified` | object | optional | both | A known-good snapshot for regression bisecting: `{ date, launcher_version, mod_version, client, parts: {name: asset-path}, by, note }`. Stamped when the mod is confirmed working in-game, so a later break has an exact point to snap back to. The launcher ignores it. |
<!-- schema-table-end -->
